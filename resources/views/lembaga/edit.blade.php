    ...
    <div class="py-2">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8 space-y-6">
            <div class="p-4 sm:p-8 bg-white shadow sm:rounded-lg">
                <div class="max-w-xl">
                    <h3 class="mb-4">{{ __('Edit Lembaga') }}</h3>
                    <form method="POST" action="{{ route('lembaga.update', $lembaga->id) }}">
                        @csrf
                        @method('PUT')
                        <div class="mb-4">
                            <label for="nama_lembaga" class="block text-sm font-medium text-gray-700">{{ __('Nama Lembaga') }}</label>
                            <input type="text" name="nama_lembaga" id="nama_lembaga" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-purple-500 focus:border-purple-500 sm:text-sm" required value="{{ old('nama_lembaga', $lembaga->nama_lembaga) }}">
                        </div>
