    ...
    <div class="py-2">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8 space-y-6">
            <div class="p-4 sm:p-8 bg-white shadow sm:rounded-lg">
                <div class="max-w-xl">
                    <h3 class="mb-4">{{ __('Edit Siswa') }}</h3>
                    <form method="POST" action="{{ route('siswa.update', $siswa->id) }}" enctype="multipart/form-data">
                        ...
                        <div class="mb-4">
                            ...
                            @if($siswa->foto)
                                <div class="mb-2">
                                    <img src="{{ asset('storage/' . $siswa->foto) }}" class="w-24 h-24 object-cover rounded-full">
                                </div>
                            @endif
                        ...
                        <div class="mb-4">
                            ...
                            <input type="number" name="nis" id="nis" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-purple-500 focus:border-purple-500 sm:text-sm" value="{{ $siswa->nis }}" required>
                        </div>
                        <div class="mb-4">
                            <label for="nama" class="block text-sm font-medium text-gray-700">{{ __('Nama Siswa') }}</label>
                            <input type="text" name="nama" id="nama" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-purple-500 focus:border-purple-500 sm:text-sm" value="{{ $siswa->nama }}" required>
                        </div>
                        <div class="mb-4">
                            <label for="email" class="block text-sm font-medium text-gray-700">{{ __('Email') }}</label>
                            <input type="email" name="email" id="email" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-purple-500 focus:border-purple-500 sm:text-sm" value="{{ $siswa->email }}" required>
                        </div>
                        <div class="mb-4">
                            ...
                            <select name="lembaga_id" id="lembaga_id" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-purple-500 focus:border-purple-500 sm:text-sm" required>
                                ...
                                @foreach($lembagas as $lembaga)
                                    <option value="{{ $lembaga->id }}" {{ $siswa->lembaga_id == $lembaga->id ? 'selected' : '' }}>{{ $lembaga->nama_lembaga }}</option>
                                @endforeach
                            </select>
                        </div>
